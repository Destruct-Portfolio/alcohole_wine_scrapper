...
export default class Handler {
     ...
     */
    static splitArrayAt200(arr) {
        let result = [];
        for (let i = 0; i < arr.length; i += 200) {
            result.push(arr.slice(i, i + 200));
        }
        return result;
    }
    static async exec() {
        ...
        if (LinksInTheFile.length === 1) {
            ...
            let WebSiteLinks = JSON.parse(fs.readFileSync('../config.json').toString());
            ...
            for (const WebLink of WebSiteLinks[0].links) {
